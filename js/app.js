...
function renderRestaurant(){
    ...
    allP.forEach(p=>{if(p.lunch){const n=getItemName(p.lunch);dc[n]=(dc[n]||0)+1}if(p.hoofdgerecht){let n=getItemName(p.hoofdgerecht);if(p.vegetarisch)n+=' (vega)';if(p.opBrood)n+=' (op brood)';dc[n]=(dc[n]||0)+1}p.extras.forEach(e=>{const n=getItemName(e);dc[n]=(dc[n]||0)+1})});
    ...
    const lD=Object.entries(dc).filter(([n])=>ALL_ITEMS.filter(i=>i.id.startsWith('lunch-')).some(i=>i.name===n)).sort((a,b)=>b[1]-a[1]);
    const eD=Object.entries(dc).filter(([n])=>ALL_ITEMS.filter(i=>i.id.startsWith('extra-')).some(i=>i.name===n)).sort((a,b)=>b[1]-a[1]);
    const skip=ALL_ITEMS.filter(i=>i.id.startsWith('lunch-')||i.id.startsWith('extra-')).map(i=>i.name);
    const hD=Object.entries(dc).filter(([n])=>!skip.includes(n.replace(' (vega)','').replace(' (op brood)',''))).sort((a,b)=>b[1]-a[1]);
    ...
    document.getElementById('summary-content').innerHTML=`
        ...
        ${lD.length?`<div class="mb-6"><h4 class="section-header mb-2.5 px-1 flex items-center gap-1.5">${micon('brunch_dining',16,'#8B5A8E')} 12 Uurtje</h4><div class="space-y-0.5">${lD.map(([n,c])=>row(n,c)).join('')}</div></div>`:''}
...
function renderPersonMenu(){
    ...
    document.getElementById('person-menu-container').innerHTML=`
        <div class="person-card ${k?'kind':''} p-5 fade-in">
            ...
            <section class="mb-7"><h4 class="section-header mb-3 flex items-center gap-1.5">${micon('brunch_dining',16,'#8B5A8E')} 12 Uurtje</h4><div class="space-y-2">${(k?MENU.lunch.kids:MENU.lunch.adult).map(i=>radioCard('lunch',i,p.lunch===i.id)).join('')}</div></section>
            <section class="mb-7"><h4 class="section-header mb-3 flex items-center gap-1.5">${micon('restaurant',16,'#8B5A8E')} Hoofdgerecht</h4>${k?kidsHTML(p):adultHTML(p)}</section>
...
function hoofdRadio(item,p){return`<div class="radio-card"><input type="radio" name="hoofd" id="hoofd-${item.id}" value="${item.id}" ${p.hoofdgerecht===item.id?'checked':''} onchange="onHoofdChange('${item.id}')" class="hidden"><label for="hoofd-${item.id}" class="flex items-start gap-3.5 p-4 cursor-pointer"><div class="dot w-5 h-5 rounded-full border-2 border-lav-300 mt-0.5 shrink-0 transition-all"></div><div class="flex-1 min-w-0"><div class="ts-title-sm text-surf-on">${esc(item.name)}</div>${item.desc?`<div class="ts-body-sm text-surf-onvar mt-1">${esc(item.desc)}</div>`:''}${item.vegOption?`<label class="inline-flex items-center gap-1.5 mt-2.5 cursor-pointer" onclick="event.stopPropagation()"><input type="checkbox" id="veg-${item.id}" ${p.vegetarisch&&p.hoofdgerecht===item.id?'checked':''} class="w-4 h-4 rounded border-lav-300 accent-pur"><span class="ts-body-sm text-lav-600">${micon('eco',14,'#A98ADB')} Vegetarisch</span></label>`:''}</div></label></div>`}
...
function renderReview(){
    document.getElementById('review-content').innerHTML=`<div class="ts-label-lg text-pur mb-4">${esc(formState.familyName)}</div>`+formState.persons.map((p,i)=>{
        ...
        return`<button onclick="goToStep(2,${i})" class="block w-full text-left py-3.5 ${i?'border-t border-lav-200/40':''} hover:bg-rose-50 -mx-2 px-2 rounded-xl transition-colors">
            <div class="flex items-center gap-2 mb-1.5"><span class="ts-title-sm text-surf-on">${esc(p.name)}</span><span class="ts-label-sm px-2 py-0.5 rounded-full ${p.isKind?'bg-lav-100 text-lav-700':'bg-rose-100 text-rose-500'}">${p.isKind?'Kind':'Volw.'}</span><span class="ml-auto ts-label-sm text-pur">wijzig →</span></div>
            <div class="ts-body-sm space-y-0.5 text-surf-on">${items.length?items.map(i=>`<div>${i}</div>`).join(''):`<div class="text-red-500 font-medium bg-red-50 px-3 py-2 rounded-xl ts-body-sm flex items-center gap-1">${micon('warning',16,'#ef4444')} Nog niks gekozen</div>`}${p.opmerkingen?`<div class="ts-label-sm text-lav-500 mt-1.5 flex items-center gap-1">${micon('edit_note',14,'#A98ADB')} ${esc(p.opmerkingen)}</div>`:''}</div>
        </button>`}).join('');
}
function saveOrder(){
    savePersonFromDOM();const inc=formState.persons.filter(p=>!p.lunch&&!p.hoofdgerecht);
...
function downloadExcel(){
    ...
    const rows=[];orders.forEach(o=>o.persons.forEach(p=>{let h=p.hoofdgerecht?getItemName(p.hoofdgerecht):'';if(p.vegetarisch)h+=' (veg)';if(p.opBrood)h+=' (op brood)';rows.push({'Naam':o.familyName,'Persoon':p.name,'Volw./Kind':p.isKind?'Kind':'Volwassene','12 Uurtje':p.lunch?getItemName(p.lunch):'','Hoofdgerecht':h,"Extra's":p.extras.map(e=>getItemName(e)).join(', '),'Dieetwensen':p.opmerkingen||''})}));
